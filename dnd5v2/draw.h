...
enum draw_event_s {
	InputSymbol = 1, InputTimer, InputIdle, InputUpdate, InputNoUpdate,
...
struct hotinfo {
	...
	const void*			object; // command context or parameters
...
struct surface {
	...
	bool				read(const char* url, color* result_pallette, int need_bpp);
